...
font-size: 85%;\
  ...
  eng2pWindow = docs.createModal("eng2p", "English Second Person Plural").size(475, 300).center();
  
		
		//docManager.createOptionToggle('Texanize plurals', 'texan', true);

		var eng2pConfigBlock =
			$('<div class="config-options" id="config-eng2p">' + 
        // '<h3>English Second Person Plural</h3>' +   
        '<a href="http://bit.ly/texas-bible-plugin" class="lightbox">Learn more about this plugin</a>' +  
    ...
    eng2pWindow.content.append(eng2pConfigBlock);
    
    
  	// add button to config
  	docManager.createEng2pPlugin = function(title, prefix) {
  		...
  		$('<input type="button" value="Configure">')
  			.on('click', function(e) {
  				if (eng2pWindow.window.is(':visible')) {
  					eng2pWindow.hide();
  				} else {
  					eng2pWindow.show();
  				}					
  	...
  	docManager.createEng2pPlugin('English Second Person Plural', 'eng2p');
