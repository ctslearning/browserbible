...
docs.DocManager = {
	...
	addDocument: function(navigator, selectedDocumentId) {
		...
		var document = new docs.Document(this, this.docIndex.toString(), navigator, selectedDocumentId);	
    ...
    if (navigator.name == 'bible') {
     document.lockBtn.addClass('state-locked'); 
    }
    ...
    //Load verse from URL
    if (urlParm(window.location, 'verse') != 'null') {
      verse = urlParm(window.location, 'verse');
      History.replaceState({verse: verse}, historyTitle(verse), '?verse=' + verse);
    }
      ...
      switch (docSetting.type) {
        case 'bible':
          if (verse != null) {
            docSetting.options.location = verse;  
          }
					
    			if (docSetting.options.location != undefined) {
    				var docu = docs.DocManager.addDocument(bible.BibleNavigator, docSetting.options.version);

            if (docSetting.options.linked) {
              docu.lockBtn.addClass('state-locked');
            }
        
            docu.navigateById(docSetting.options.location, false);
    			}
          
          break;
          
        case 'resource':
          var resources = new resourcesClass(t);
          resources.loadResource(docSetting.options.resourceID);
          break;
      ...
      switch (document.type) {
        case 'bible':
          options = {
    				...
    				linked: document.lockBtn.hasClass('state-locked')
    			};
          break;
        case 'resource':
          options = {
	...
	sync: function(sourceDocument, visibleFragmentInfo) {
      ...
      var correctDoc = (sourceDocument != null && sourceDocument.id != document.id // make sure this isn't the one that generated the request
			...
			if ((correctDoc || sourceDocument == null) && document.lockBtn != undefined && document.lockBtn.hasClass('state-locked')) {
          
        var fragmentId = visibleFragmentInfo.fragmentId || visibleFragmentInfo;
        var offset = visibleFragmentInfo.offset || 0;
				
				document.navigateById(fragmentId, false, offset);
...
docs.Document = function(manager, id, navigator, selectedDocumentId) {
  ...
  t.type = 'bible';
	...
	// create pane    
  t.container = $(_template('docs_document_container', {id: t.id, options: t.navigator.getOptions(), classes: ['bible'].join(' ')}));
  ...
  t.version = t.selector.val();
  t.versionInfo = bible.versions.versionsByKey[t.version];
	...
	t.settingsBtn = t.container.find('.document-settings-button');
  t.buttonsContainer = t.container.find('.document-header-right-buttons');
...
docs.Document.prototype = {
    ...
    //Let CSS know how many documents we have open.
    ...
    .addClass('documents-' + docs.DocManager.documents.length);
    ...
    //reset settings toolbar left.
    t.buttonsContainer.removeAttr('style');
    ...
    //Sync other linked docs.
    // if (t.lockBtn.hasClass('state-locked')) {
    //   t.manager.sync(t, fragmentId);
    // }
