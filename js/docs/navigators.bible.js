...
bible.BibleNavigator = {
	...
	getOptions: function() {
      ...
      // html += '<optgroup label="' + language.languageName + '">';
			...
			for (versionCode in language.versions) {
				...
				html += '<option value="' + versionCode + '" data-language="' + langCode + '" data-version=\'' + JSON.stringify(version) + '\'>' + version.abbreviation + ' ('+ language.languageName + ')</option>';
	...
	setupNavigationList: function(document) {
      ...
      //Navigate to book. 
      var bookOsis = li.attr('data-osis');
      var destination = bookOsis + ' 1';
			document.navigateByString(destination);
      if (document.lockBtn.hasClass('state-locked')) {
        document.manager.sync(document, document.navigator.parseString(destination));
      }
      ...
      var destination = bookOsis + ' ' + chapterNumber;
			document.navigateByString(destination);
    ...
    var versionInfo = document.versionInfo;
		var booksHtml = '';
    ...
    //No more errors, but it doesn't show the window still.
