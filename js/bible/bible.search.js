...
bible.BibleSearch = {
	...
	search: function(text, version, chapterCallback, foundCallback, endedCallback) {
    ...
    s.searchResultsBooks = {};
		...
		s.isLemmaSearch = /(G|H|g|h)\d{1,5}/.test(s.searchText);
		
		if (s.isLemmaSearch) {
		
			// just search the desired testament
			if (s.searchText.substring(0,1) === 'H' || s.searchText.substring(0,1) === 'h') {
				s.bookRange = bible.OT_BOOKS;
			} else if (s.searchText.substring(0,1) === 'G' || s.searchText.substring(0,1) === 'g') {
